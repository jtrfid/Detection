...
\node [int] (c) [right of=b] {$\bm{x}_N$};
\node [int] (d) [below of=c] {$\lambda(\bm{x}_N)\frac{p(\bm{x}_N|H_1)}{p(\bm{x}_N|H_0)}\mathop{\gtrless}\limits_{H_0}^{H_1}\eta$};
\node [int] (e) [left of=d] {$l[x(t)])\mathop{\gtrless}\limits_{H_0}^{H_1}\gamma$};
\path[->] (a) edge node {$\{f_k(t)\}$正交展开} (b);
\path[->] (b) edge node {前N项$x_k$构成$\bm{x}_N$} (c);
...
\begin{frame}[shrink]{简单二元信号波形检测要点}
\begin{columns}
	\column{0.4\textwidth}
	\begin{align*}
	&H_0: x(t)=n(t)\\
