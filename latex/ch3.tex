...
\begin{columns}
	...
	\scriptsize
	\textcolor{blue}{$p(l|H_j)(j=0,1)$: 假设$H_j$下观测信号的概率密度函数; $r^+=\frac{\sigma^2\ln\eta}{NA}+\frac{A}{2}; \alpha=P(H_1|H_0)$}
\end{columns}
...
Q(x)是单调递减函数,其反函数用$Q^{-1}[\bullet]$表示。 因为
	\[P(H_1|H_0)=Q(\ln\eta/d+d/2) \implies \ln\eta/d=Q^{-1}[P(H_1|H_0)]-d/2 \]
这样有:
\begin{align*}
P(H_1|H_1)&=Q\left(\ln\eta/d-d/2\right)\\
&=Q[Q^{-1}[P(H_1|H_0)]-d/2-d/2]=Q[Q^{-1}[P(H_1|H_0)]-d/2-d]
\end{align*}
